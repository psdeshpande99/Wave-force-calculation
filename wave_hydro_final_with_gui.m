...
for i =1:length(z)
    ...
    Ft1(i,:) = Fd1(i,:) + Fi1(i,:);
    Mt1(i,:) = Ft1(i,:)*z(length(z)+1-i);
end

%% Q1 b)uniform current (in the direction of wave)%%
u_current_uniform = 1;
KC_uniform_supporting = (umax+u_current_uniform)*T/D;
...
for i =1:length(z)
    ...
    Ft5(i,:) = Fd5(i,:) + Fi5(i,:);
    Mt5(i,:) = Ft5(i,:)*z(length(z)+1-i);
end

%% Q1 plots no current%%
figure('Name','No_Current','NumberTitle','off','Renderer', 'painters', 'Position', [0 0 900 150]);
subplot(2,3,1)
plot(theta,Ft1(1,:),'r');
hold on;
plot(theta, Fi1(1,:),'b');
...
figure('Name','All_cases_with_depth_variation','NumberTitle','off','Renderer', 'painters', 'Position', [0 0 900 150]);
...
[~,I1] = find(Ftotal_nocurrent == max(Ftotal_nocurrent));
[~,I2] = find(Ftotal_uniform_current_supporting == max(Ftotal_uniform_current_supporting));
[~,I3] = find(Ftotal_uniform_current_oppo == max(Ftotal_uniform_current_oppo));
[~,I4] = find(Ftotal_varying_current_supporting == max(Ftotal_varying_current_supporting));
[~,I5] = find(Ftotal_varying_current_opposing == max(Ftotal_varying_current_opposing));
figure('Name','All_cases_F_total','NumberTitle','off','Renderer', 'painters', 'Position', [0 0 900 150]);
...
%% Moment plot %%
figure('Name','All_cases_M_total','NumberTitle','off','Renderer', 'painters', 'Position', [0 0 900 150]);
...
figure('Name','Sectional total force deoth variation','NumberTitle','off','Renderer', 'painters', 'Position', [0 0 900 150]);
